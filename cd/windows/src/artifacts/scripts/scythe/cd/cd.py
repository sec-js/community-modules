...
def create_parser(db, os="windows"):
    ...
    epilog =  '  scythe.cd --dir "C:\\users\"\n'
    parser = ArgumentParser(prog="cd", description="Change directory of current and parent process",
                            epilog=epilog)
    parser.add_argument("--dir", help="Directory to change to", required=True)
    return parser
...
def create_message_body(db, command_line, campaign_name, endpoint_name):
    ...
    if not command_line:
        raise ValueError("Error: --dir argument is missing.")
    else:
...
def report(db, request, response, format_):
    ...
    return "--dir \"%s\"" % request_contents, content, "pre"
