...
rivergumsecurity.process_inject_method1 --url <shellcode URL>'''
    parser = ArgumentParser(
        prog="process_inject_method1",
        description="Inject shellcode into memory",
        epilog=epilog
    )
    parser.add_argument(
        "-u", "--url",
        help="URL to GET base64 encoded shellcode from",
        required=False
    )
    return parser
...
def create_message_body(db, command_line, campaign_name, endpoint_name):
    ...
    return args.url.strip('"').encode("utf-8")
...
def report(db, request, response, format_):
    ...
    return "--url \"%s\"" % request_contents, content, "pre"
