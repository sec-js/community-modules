...
def create_parser(db, os="windows"):
    ...
    epilog =  '  scythe.echo --message "Hello, world!"\n'
    parser = ArgumentParser(prog="echo", description="Echo the request back as response",
                            epilog=epilog)
...
def report(db, request, response, format_):
    ...
    return "--message \"%s\"" % request_contents, request_contents, "pre"
